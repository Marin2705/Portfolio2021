  ...
  return (
    ...
    id={'projectElem-'+props.i}>
      <figure className='projectFig cursor-pointer'>
        <Picture src={props.thumb} alt={props.thumbAlt} className="max-w-[60vw]"/>
        {/* <img src={props.thumb} alt={props.thumbAlt} className="max-w-[60vw]"/> */}
        <figcaption>{props.thumbCaption}</figcaption>
        <p className="font-thin lg:text-1xl">{props.text.substring(0,50)}... <br /><a href="#root" rel="opener" className="inline-block underline transition-all transform hover:scale-110">Learn more</a></p>
      </figure>
    </div>
  );
}
